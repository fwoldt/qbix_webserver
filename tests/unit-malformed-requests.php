...
$cmd = escapeshellarg(PHP_BINARY) . ' ' . escapeshellarg($phar)
	...
	. ' --port=' . $port . ' --workers=2';
$proc = proc_open($cmd . ' > ' . escapeshellarg($tmp . '/log') . ' 2>&1',
	array(), $pipes);
if (!is_resource($proc)) { fwrite(STDERR, "  FAIL - could not start\n"); exit(1); }

register_shutdown_function(function () use ($proc, $port) {
	$st = @proc_get_status($proc);
	if ($st and $st['running']) {
		// The shell wrapper means proc_terminate may reach only the shell, so
		// the server is asked to stop by pid as well.

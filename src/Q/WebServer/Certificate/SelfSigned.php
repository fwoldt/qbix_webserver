 ...
 *   - renewed RENEW_DAYS before it expires, and whenever the host names and
 *     addresses it should cover change;
...
class Q_WebServer_Certificate_SelfSigned
{
	...
	static function renewalReason($c, array $hosts)
	{
		...
		$want = array_map('strtolower', $hosts);
		sort($want);
		if ($want !== $c->hosts()) {
			return 'the host names changed (now ' . implode(' ', $want) . ')';
		}
		return null;
	}
